...
export default function AdminUsersPage() {
  ...
  const handleStatusChange = async (email: string, status: string) => {
    ...
    try {
      await axios.patch("/api/users/update-status", { email, status });
      await fetchUsers();
    } catch (err: unknown) {
      const msg =
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {user.accountStatus !== "active" && (
                            <Button
                              ...
                              className="h-7 text-xs text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-900/30"
                            >
                              {actionLoading === user.email ? <RefreshCw className="mr-1 h-3 w-3 animate-spin" /> : <CheckCircle2 className="mr-1 h-3 w-3" />}
                              Activate
                            </Button>
                          )}
                          {user.accountStatus !== "suspended" && (
                            <Button
                              ...
                              className="h-7 text-xs text-amber-600 hover:text-amber-700 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-900/30"
                            >
                              {actionLoading === user.email ? <RefreshCw className="mr-1 h-3 w-3 animate-spin" /> : <ShieldAlert className="mr-1 h-3 w-3" />}
                              Suspend
                            </Button>
                          )}
                          {user.accountStatus !== "blocked" && (
                            <Button
                              ...
                              className="h-7 text-xs text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-900/30"
                            >
                              {actionLoading === user.email ? <RefreshCw className="mr-1 h-3 w-3 animate-spin" /> : <UserX className="mr-1 h-3 w-3" />}
                              Block
                            </Button>
                          )}
                        </div>
